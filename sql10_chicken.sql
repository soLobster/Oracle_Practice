...
select max(calls) as 최댓값, min(calls) as 최솟값
from call_chicken;
-- 4. 통화건수가 최솟값이거나 최댓값인 레코드(행 전체)를 출력하세요.
select *
from call_chicken
where calls = (select min(calls) from call_chicken) or calls =(select max(calls) from call_chicken);
-- 5. 평균적으로 어떤 요일에서 치킨 주문이 많을까요?
select call_day , round(avg(calls),2) as 평균
from call_chicken
group by call_day
order by 평균 desc;
--having avg(calls) = (
--                    select max(avg_calls) 
--                    from (
--                        select call_day, avg(calls) as avg_calls 
--                        from call_chicken 
--                        group by call_day
--                        )
--                    );
-- 6. 평균적으로 어떤 연령대가 치킨 주문을 많이 할까요?
select ages, round(avg(calls),2) as 평균
from call_chicken
group by ages
order by 평균 desc;
--having avg(calls) = (
--                    select max(avg_calls)
--                     from (
--                            select ages, avg(calls) as avg_calls
--                            from call_chicken
--                            group by ages
--                            )
--                    );
-- 7. 평균적으로 어느 지역에서 치킨 주문을 많이 할까요?
select district, round(avg(calls),2) as 평균
from call_chicken
group by district
order by 평균 desc;
--having avg(calls) = (
--                    select max(avg_calls)
--                     from (
--                            select district, avg(calls) as avg_calls
--                            from call_chicken
--                            group by district
--                            )
--                    );
-- 8. 치킨 주문에 성별 차이가 있을까요?
select gender as 성별, sum(calls) as 치킨주문수, round(avg(calls),2) as 평균
from call_chicken
group by gender;
-- 9. 요일별, 연령대별 통화건수의 평균을 찾으세요.
select call_day as 요일 , ages as 연령대, round(avg(calls),2) as 평균
from call_chicken
group by call_day, ages
order by 평균 desc;
-- 10. 구별, 성별 통화건수의 평균을 찾으세요.
select district as 구별, gender as 성별, round(avg(calls),2) as 평균
from call_chicken
group by district, gender
order by 평균 desc;
-- 11. 요일별, 구별, 연령대별 통화건수의 평균을 찾으세요.
select call_day as 요일별, district as 구별, ages as 연령대별, round(avg(calls),2) as 평균
from call_chicken
